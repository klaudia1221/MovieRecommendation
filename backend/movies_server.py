...
def get_movies_recommendations(lists, n=20):  # movie_ix, rating
    ...
    # sortowanie po movieId
    lists = np.array(sorted(lists, key=lambda a_entry: a_entry[0]))
    movie_ixs = lists[:, 0]
    ...
    # 6. forward
    result = forward(mean_vec_weights, mean_vec_bias,
                     movies_weights, movies_bias)

    # 7. top 30 films, we will get 50 best movies and random sample
    size = (len(movie_ixs)+n+50)
    sample_size = (len(movie_ixs)+n)
    top40_films = result.argsort(0, descending=True)[:size].numpy()[
        np.random.choice(size, sample_size)]
